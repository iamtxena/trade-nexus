...
|----------|----------|-------------|
| `LONA_AGENT_REGISTRATION_SECRET` | No | Provided by Lona platform admin. If set, used for secret-based registration. If not set, CLI auto-falls back to invite code flow (zero-config). |
...
**Two registration methods (automatic):**
- If `LONA_AGENT_REGISTRATION_SECRET` is set → uses secret-based registration
- If not set → automatically requests an invite code and registers with it (zero-config)
...
# Data is named as SYMBOL_INTERVAL_STARTDATE_ENDDATE (e.g. BTCUSDT_1h_20250101_20250601)
...
# Re-download with --force (deletes existing symbol with same name first)
bun run nexus data download \
  --symbol BTCUSDT \
  --interval 1h \
  --start 2025-01-01 \
...
bun run nexus data delete --id <symbol-id>
```
...
| Data download name conflict | Use `--force` flag to auto-delete existing symbol, or `nexus data delete --id <id>` |
| `LONA_AGENT_REGISTRATION_SECRET` missing | Not needed — CLI auto-uses invite code flow if secret is not set |
