...
REQUIRED_GATE_REFS = ["#76", "#77", "#78", "#79", "#80", "#81", "#106"]
...
def main() -> int:
    ...
    traceability_chunk = ROOT / "docs/llm/chunks/portal_traceability_v1.md"
    if traceability_chunk.exists():
        chunk_text = traceability_chunk.read_text(encoding="utf-8")
        for gate_ref in REQUIRED_GATE_REFS:
            if gate_ref not in chunk_text:
                errors.append(f"Traceability chunk missing gate reference: {gate_ref}")
    else:
        errors.append("Missing traceability chunk: docs/llm/chunks/portal_traceability_v1.md")
